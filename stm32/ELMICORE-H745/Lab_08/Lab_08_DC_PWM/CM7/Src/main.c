...
int main(void)
{
 ...
 /* STM32H7xx HAL library initialization:
  ...
  /* Add Cortex-M7 user application code here */
    ...
    GPIO_InitTypeDef  gpio_init_structure_dir;

    gpio_init_structure_dir.Pin   = GPIO_PIN_6;
    gpio_init_structure_dir.Mode  = GPIO_MODE_OUTPUT_PP;
    gpio_init_structure_dir.Pull  = GPIO_PULLDOWN;
    gpio_init_structure_dir.Speed = GPIO_SPEED_FREQ_VERY_HIGH;

    HAL_GPIO_Init(GPIOA, &gpio_init_structure_dir);
  ...
  /* Infinite loop */
  bool flag = GPIO_PIN_SET;
  while (1)
  {
	  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, flag);
	  ...
	  while (pwm <= 100) {
		  __HAL_TIM_SET_COMPARE(&TimHandle, TIM_CHANNEL_1, PULSE_SCALE * pwm);
		  pwm++;
		  HAL_Delay(30);
	  }
