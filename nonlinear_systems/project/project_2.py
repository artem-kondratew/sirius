...
def rhs(t, x):
    ...
    c1 = 10
    c2 = 9
    gamma1 = y3 + c1 * y2 + c2 * y1
    gamma2 = y4 + c1 * y3 + c2 * y2

    continuous_controller = False

    if continuous_controller:
        nu += -1.1 * Lc * sign_exp(gamma1, 0) * dt
        theta2 = -2.7 * Lc ** (2/3) * sign_exp(gamma1, 1/3) - 5.345 * Lc ** (1/2) * sign_exp(gamma2, 1/2) + nu
    else:
        theta2 = -1.1 * Ld * sign_exp(gamma1, 0) - 1.5 * Ld ** (1/2) * sign_exp(gamma2, 0)

    v2 = theta2 - (c1 * y4 + c2 * y3) / a41 / b
