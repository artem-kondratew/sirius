...
def rhs(t, x):
    ...
    if continuous_controller:
        nu += -0.002 * Lc * sign_exp(y1, 0) * dt
        v4 = -1.1 * Lc ** (4/5) * sign_exp(y1, 1/5) - 1.9 * Lc ** (3/4) * sign_exp(y2, 1/4) - 2.6 * Lc ** (2/3) * sign_exp(y3, 1/3) - 2.8 * Lc ** (1/2) * sign_exp(y4, 1/2) + nu
    else:
        v4 = -1.1 * Ld * sign_exp(y4 + 3 * (y3 ** 6 + y2 ** 4 + np.abs(y1) ** 3) ** (1/12) * sign_exp(y3 + (y2 ** 4 + np.abs(y1) ** 3) ** (1/6) *
                                                                                                sign_exp(y2 + 0.5 * np.abs(y1) ** (3/4) * sign_exp(y1, 0), 0), 0), 0)
